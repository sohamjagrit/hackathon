...
export function VoicePanel({ onReset }: VoicePanelProps) {
  ...
  return (
    <div style={{ width: 300, flexShrink: 0, display: 'flex', flexDirection: 'column', background: 'oklch(0.985 0.006 75)', borderRight: '1px solid oklch(0.9 0.01 75)' }}>

      {/* Header */}
      <div style={{ padding: '22px 20px 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid oklch(0.92 0.01 75)' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11.5, color: 'oklch(0.55 0.01 75)' }}>your trip, spoken into being</div>
        </div>
        <button
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {transcript.length === 0 && (
          <div style={{ fontSize: 12.5, color: 'oklch(0.65 0.01 75)', textAlign: 'center', marginTop: 24, lineHeight: 1.6 }}>
      ...
      {/* Mic controls */}
      <div style={{ padding: '14px 20px 20px', borderTop: '1px solid oklch(0.92 0.01 75)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
        <div style={{ height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
          {isConnected && (
            <>
              {[1,2,3,4].map(n => (
                <span key={n} style={{ width: 3, height: 14, background: 'oklch(0.45 0.09 165)', borderRadius: 2, animation: `bar${n <= 2 ? n : n - 2} .6s infinite ease-in-out${n > 2 ? ' .2s' : ''}` }} />
              ))}
            </>
          )}
        </div>
        ...
        <div style={{ fontSize: 12, color: 'oklch(0.55 0.01 75)' }}>{micLabel}</div>

        {isDone && (
          <button
            onClick={onReset}
            style={{ border: '1px solid oklch(0.85 0.01 75)', background: 'none', color: 'oklch(0.5 0.01 75)', fontSize: 12, padding: '5px 12px', borderRadius: 20, cursor: 'pointer', fontFamily: 'inherit' }}
          >
            Start over
          </button>
        )}
